...
from karteikartenapi import stats
from auth import (
    current_user,
    identity
)
...
@app.route('/auth/handler', methods=['GET'])
def login():
...
def register():
    ...
    try:
        payload = jwt.decode(id_token, verify=False)  # TODO production: verify must be True
        ...
        access_token = jwt.encode(credentials.signer, {
            'sub': user.key.id(),
        })
    ...
    if old_media_id == updated_media_id:
        ...
        if updated_media is not None:
            ...
            if card_id not in card_ids:
                updated_media.populate(card_ids=card_ids + [ndb.Key(models.Card, card_id)])
                updated_media.put()
...
def list_collections():
    ...
    user_id = identity()
    collections, next_cursor, more = models.Collection.query_by_user_id(user_id).fetch_page(20)
...
@app.route('/collections/recent', methods=['GET'])
...
@app.route('/me/scorecard/<int:card_id>', methods=['PATCH'])
...
@app.route('/me/recent-collections', methods=['PATCH'])
